...
const fetchDocuments = async () => {
  ...
  message.value = null
  try {
    const res = await documentStore.getDocument()
    documents.value = res?.documents ?? []
    toastSuccess(res?.message || 'Documents chargés avec succès ✅')
  } catch (err: any) {
    console.error('Erreur lors du chargement des documents:', err)
    error.value = 'Impossible de charger les documents. Veuillez réessayer.'
...
const deleteDocument = async (id: number) => {
  ...
  try {
    ...
    toastError(err.response?.data?.message || 'Impossible de supprimer ce document')
  }
}
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <span
                :class="[
                  ...
                  { 'bg-orange-100 text-orange-800': document.is_validated === 1 },
                  { 'bg-purple-100 text-purple-800': document.is_validated === 0 },
                ]"
              >
                {{ document.is_validated === 0 ? 'En attente de validation' : 'Validé' }}
              </span>
            </td>
