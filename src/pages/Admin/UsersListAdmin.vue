...
const isOpenModalEditUser = ref<boolean>(false)
const isOpenModalCreateUser = ref<boolean>(false)
...
const addUser = () => {
  isOpenModalCreateUser.value = true
}
...
const deleteUser = async (id: number) => {
  try {
    ...
    toastSuccess(res?.message || ' avec succès')
  } catch (err: any) {
    ...
    <div v-else class="bg-white p-6 rounded-lg shadow-md overflow-x-auto">
      ...
      >
        <div class="relative w-full max-w-3xl bg-white rounded-2xl shadow-2xl p-8">
          ...
          <!-- Contenu de l’édition -->
          <EditUser @updated="handleUserUpdated" :form="userData!" :id="userData?.id" />
        </div>
      </div>
